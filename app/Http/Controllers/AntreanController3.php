...
class AntreanController extends Controller
{
    ...
    public function create(Request $request)
    {
      ...
      public function list_antrean(Request $request)
    {
    	$validator = Validator::make($request->all(), [
            'tanggalperiksa'  => 'date_format:Y-m-d|after_or_equal:today|required',
        ]);
        
        $list_antrean = Antrean::where('tanggal_periksa', $request->tanggalperiksa)
                                ->whereRaw('LOWER(status) != "batal"')
                                ->get();

        $response = array();
        foreach ($list_antrean as $key => $value) {
            $resp = [
                'nomorantrean' => $value->no_antrean,
                'kodebooking' => $value->kode_booking,
                'namapoli' => $value->kode_poli,
                'namadokter' => $value->kode_dokter,
                'nik' => $value->nik,
                'nokartu' => $value->no_kartu,
                'notelp' => $value->no_telp,
                 'norujukan' => $value->no_referensi,
                 'status' => $value->status,
    ...
    public function ambil(Request $request)
    {
       ...
       if ($last_antrean) {
            ...
            $stamp = strtotime("+15 minutes", strtotime($request->tanggalperiksa));
        }   
