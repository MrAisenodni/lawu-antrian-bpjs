...
class AntreanController extends Controller
{
    ...
    //    $count_antrean = Antrean::where('tanggal_periksa', $request//->tanggalperiksa)
      ...
      //       $stamp = strtotime("+$estimasi minutes", strtotime($last_antrean //->tanggal_periksa));
       // } else {
          ...
          // $stamp = strtotime(date('Y-m-d', strtotime($request->tanggalperiksa//)) . ' ' . date('H-i-s', strtotime($last_time_doctor->jammulai//)));
           
          // $stamp = strtotime(date('H-i-s', strtotime($last_time_doctor->jammulai)), strtotime($request->tanggalperiksa));
   ...
   //    }               
     ...
     // if($response_code=="200")
        //{
            //return AppHelper::response_json(null,201,$response_code);
       // }
      ...
      // $data->no_antrean = $request->jkn. '' . $next_antrean;
       ...
       // $data->kode_booking = $request->kodepoli . '-' . date('YmdHis');
        ...
        $data->estimasi_dilayani = date('Y-m-d H:i:s', $stamp);
         $data->waktu = strtotime($data->estimasi_dilayani);
        ...
        $response = [
            ...
            'namapoli'=>$poli_hari_ini->nama_poli,
            'namadokter'=>$poli_hari_ini->nama_dokter,
            'estimasidilayani'=>$stamp * 1000,//strtotime($data->estimasi_dilayani),
            'sisakuotajkn'=>$response_sisajkn * 1,
            'kuotajkn'=>$response_jkn * 1,
            'sisakuotanonjkn'=>$response_sisanonjkn * 1,
            'kuotanonjkn'=>$response_nonjkn * 1,
            'keterangan'=>'Tunjukkan Pada Petugas Saat Akan Periksa.',//$data->keterangan,
        ];
